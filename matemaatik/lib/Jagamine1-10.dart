...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const MaterialApp(
      home: PageThirdyeight(),
      debugShowCheckedModeBanner: false,
    );
  }
}

class PageThirdyeight extends StatefulWidget {
  const PageThirdyeight({Key? key}) : super(key: key);
...
class _MyAppState extends State<PageThirdyeight> {
...
List<Map<String, Object>> _generateQuestions() {
  ...
  for (int i = 1; i <= 10; i++) {
    int divisor = Random().nextInt(10) + 1; // Add 1 to ensure non-zero values
